"""RaulNetV19: V18 with the spatial-temporal conv factorized (R(2+1)D-style).

The dense (grids, electrodes/2, 18) conv was 94% of the encoder forward.
Factorizing it into a spatial conv followed by a temporal conv (GELU between)
is 4.7x faster on the layer and 2.0x end-to-end in training, at unchanged
accuracy (held-out R2 0.890 vs 0.891 dense, Sub1 8-task benchmark, RTX 4090,
36.9 vs ~75 min wall-clock). Encoder output width is unchanged; state dicts
are NOT compatible with V17/V18 (conv2 is now two modules).
"""
...
class RaulNetV19(L.LightningModule):
